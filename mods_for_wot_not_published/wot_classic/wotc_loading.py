...
from gui.game_loading.state_machine.states.slide import _showImage
...
def ClientLoadingState_configure(self, preferences, images, texts, progressSetting, viewSettings):
    CLSconf_base(self, preferences, images, texts, progressSetting, viewSettings)
    self.mainState.setImage(getLoader()._cdnImages.get())
...
def LoginScreenState_onEntered(self):
    ...
    self._image = getLoader()._cdnImages.get()
...
def LoginView_populate(self):
    LVP_base(self)
    self.as_setVersionS('#wek_gameLoading:game_version')
    self.as_setCopyrightS('11111111111111111111111111111111111111111', '11111111111111')
    self.flashObject.copyright.copyright.text = '#wek_gameLoading:copyright'

CLPSC_base = ClientLoadingProgressStateComponent._setProgress
CLSconf_base = ClientLoadingState.configure
LVP_base = LoginView._populate

ClientLoadingProgressStateComponent._setProgress = ClientLoadingProgressStateComponent_setProgress
LoginView._populate = LoginView_populate

BaseViewResourcesTickingState._task = lambda _: None
ClientLoadingState.configure = ClientLoadingState_configure
...
LoginScreenState.setImage = lambda _, __: None
